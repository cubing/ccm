...
function time(ms) {
  return { millis: ms, wcaValue: Math.floor(ms/10), decimals: 2 };
}

function moves(mvs) {
  return { moveCount: mvs, wcaValue: mvs };
}

function dnf() {
  return { puzzlesSolvedCount: 0, puzzlesAttemptedCount: 1 };
}

function dns() {
  return { puzzlesSolvedCount: 0, puzzlesAttemptedCount: 0 };
}
  ...
  describe('computeSolvesStatistics', function() {
    describe('average of five', function() {
      ...
      it('simple average', function() {
        ...
        var stats = wca.computeSolvesStatistics(solves, roundFormatCode, 'f');
      ...
      it('average with double DNF', function() {
        ...
        var stats = wca.computeSolvesStatistics(solves, roundFormatCode, 'f');
        expect(stats.average).to.equal(dnf());
        expect(stats.bestIndex).to.equal(0);
      ...
      it('average with soft cutoff', function() {
        ...
        var stats = wca.computeSolvesStatistics(solves, roundFormatCode, 'f');
        expect(stats.average.wcaValue).to.equal(0);
        expect(stats.bestIndex).to.equal(0);
        expect(stats.worstIndex).to.equal(1);
      });

      it('average with soft cutoff (zeroes) ???', function() {
        var solves = [time(1200), time(3200), time(0), time(0), time(0)];
        var stats = wca.computeSolvesStatistics(solves, roundFormatCode, 'f');
        expect(stats.average.wcaValue).to.equal(0);
        expect(stats.bestIndex).to.equal(0);
        expect(stats.worstIndex).to.equal(1);
    ...
    describe('mean of three', function() {
      ...
      it('simple mean', function() {
        ...
        var stats = wca.computeSolvesStatistics(solves, roundFormatCode, 'f');
        expect(stats.average.millis).to.equal(2200);
        expect(stats.bestIndex).to.equal(1);
    ...
    describe('best of three', function() {
      ...
      describe('fmc', function() {
        it('simple fmc', function() {
          ...
          var stats = wca.computeSolvesStatistics(solves, roundFormatCode, 'f');
          expect(stats.average).to.deep.equal({ solveCount: 3600, wcaValue: 0 });
          expect(stats.bestIndex).to.equal(0);
          expect(stats.worstIndex).to.equal(1);
        });
    ...
    describe('best of one', function() {
      ...
      describe('multiblind', function() {
        it('(later)', function() {
          expect('write later').to.equal('not now');
        });
      });
    });
